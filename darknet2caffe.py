...
def load_conv2caffe(buf, start, conv_param):
    ...
    #conv_param[0].data[...] = np.resize(buf[start:start+weight.size], weight.shape); start = start + weight.size
    #if(buf[start:start+weight.size].shape ==(127999,)):
    #conv_param[0].data[...] = np.resize(buf[start:start+weight.size], weight.shape); start = start + weight.size
...
def cfg2prototxt(cfgfile):
    ...
    for block in blocks:
        if block['type'] == 'net':
            ...
            if 'name' in block:
                reshape_layer['name'] = block['name']
                reshape_layer['top'] = block['name']    
            else:
            ...
            block_n_idx_tuple_list = list(map(lambda b, idx: \
                                            (b['type'], idx), \
                                          blocks, range(len(blocks))))
            route_tuple_list = list(filter(lambda t: \
                                         t[0] == "route",\
            ...
            if first_route_block['type'] == "route":
                ...
                if len(first_route_from_layers_list) == 1:
                    ...
                    second_route_from_layers_idx_list = list(map(lambda ind_str: 
                    ...
                    print("\n=============== check master branch =================")
                    for mbidx in range(len(master_branch_blocks)): 
                        master_block = master_branch_blocks[mbidx]
                        print("%d\t%s" % (mbidx, str(master_block)))

                    print("\n=============== check right branch ==================")
                    for rbidx in range(len(right_branch_blocks_only_before_reorg)):
                    ...
                    for mridx in range(len(master_nd_right_branch_blocks)):
                    ...
                    out_h = int(input_h / int(block['stride']))
                    out_w = int(input_w / int(block['stride']))
